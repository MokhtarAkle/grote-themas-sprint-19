...
<main>
  <section>
    <div class="upload-button">
      ...
      <a href="/upload"
        >Ben jij docent en wil je zelf een werkvorm uploaden? Klik dan hier om
        je aan te melden.</a
      >
    </div>
    ...
    {#if workform.video == null}
      {#if workform.thumbnail_performant}
        <img
          class="thumbnail"
          src={"https://platform-big-themes.directus.app/assets/" +
            workform.thumbnail_performant.id}
          alt={workform.alt}
          loading="lazy"
    ...
    {:else}
      <video
        controls
        poster={"https://platform-big-themes.directus.app/assets/" +
          workform.thumbnail.id}
        alt={workform.alt}
        width="480"
        height="480"
    ...
    <article class="content">
      ...
      <div class="files">
        <h3>Bestanden</h3>
        {#if workform.files[0]}
          {#each workform.files as file}
			<div>
			<h4>{file.directus_files_id.title}</h4>
            <iframe
              src={"https://platform-big-themes.directus.app/assets/" +
            ...
            ></iframe>
            <a
              href={"https://platform-big-themes.directus.app/assets/" +
                file.directus_files_id.id}
              download>Download</a
            >
		</div>
        ...
        <div class="tag-list">
          {#if workform.sub_tags.length > 0}
            {#each workform.sub_tags as tag}
              <div
                class="single-tag"
                style="border-color: {tag.sub_tag_id.color};"
              >
